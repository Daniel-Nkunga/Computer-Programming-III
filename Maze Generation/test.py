...
def show_maze(maze, exploring, wait=1):
    # Create a copy of the maze to overlay exploration highlights
    maze_with_highlight = cv2.cvtColor(maze, cv2.COLOR_GRAY2BGR)
    
    # Highlight cells being explored in blue
    for row, col in exploring:
        cv2.rectangle(maze_with_highlight, (col * 20, row * 20), ((col + 1) * 20, (row + 1) * 20), (255, 0, 0), -1)
    
    # Display the maze with exploration highlights
    cv2.imshow('Maze', maze_with_highlight)
    cv2.waitKey(wait)
...
def dfs(maze, visited, row, col, target, path):
    ...
    for dr, dc in directions:
        ...
        if is_valid_move(maze, new_row, new_col, visited):
            ...
            show_maze(maze, path)  # Show maze with exploration highlight
...
def kruskal_maze(maze):
    ...
    for weight, (y1, x1), (y2, x2) in edges:
        ...
        if root1 != root2:
            ...
            show_maze(maze, [(y1, x1), (y2, x2)])  # Show maze with exploration highlight
            cv2.waitKey(1)
...
def main():
    ...
    if path:
        for pos in path:
            ...
            show_maze(maze_with_path, [])  # Show final path without exploration highlight
